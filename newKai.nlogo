...
breed [nodes node ]
breed [tasks task ]
nodes-own [ max-capacity stack-of-tasks capability-a capability-b capability-c working-on]
...
to setup-nodes
  create-nodes number-of-nodes [
    ...
    set capability-a 0.4 + (random-float 0.2);
    set capability-b 0.4 + (random-float 0.2);
    set capability-c 0.4 + (random-float 0.2);
  ]
end
...
to assign-task-to-node  [ #t #n ]
  ask #n [
    let task-time 0
    if [ task-type ] of #t = "a" [
      set task-time ([ difficulty ] of #t) / capability-a
    ]
    if [ task-type ] of #t = "b" [
      set task-time ([ difficulty ] of #t) / capability-b
    ]
    if [ task-type ] of #t = "c" [
      set task-time ([ difficulty ] of #t) / capability-c
    ]
    ask #t [
      let result ceiling task-time
      set time ceiling result
    ]
  ...
  ask #a [
    ...
    ifelse working-on != nobody [
      ifelse [ time ] of working-on = 0 [
        if [ task-type ] of working-on = "a" [
          let old capability-a
          set capability-a capability-a + (learning-rate / 100) * (1 - capability-a) * [difficulty] of working-on
          show (word "Task of type a and difficulty " [ difficulty ] of working-on " completed - increased capability from " old " to " capability-a)
        ]
        if [ task-type ] of working-on = "b" [
          let old capability-b
          set capability-b capability-b + (learning-rate / 100) * (1 - capability-b) * [difficulty] of working-on
          show (word "Task of type b and difficulty " [ difficulty ] of working-on " completed - increased capability from " old " to " capability-b)
        ]
        if [ task-type ] of working-on = "c" [
          let old capability-c
          set capability-c capability-c + (learning-rate / 100) * (1 - capability-c) * [difficulty] of working-on
          show (word "Task of type c and difficulty " [ difficulty ] of working-on " completed - increased capability from " old " to " capability-c)
        ]
        ask working-on [
          die
        ]
