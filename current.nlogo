globals [ task-movement-in-progress ]

breed [nodes node ]
breed [task-turtles task-turtle ]
nodes-own [ layer number success-probability task-assigned task-completed ]
task-turtles-own [ task-target ]
...
to setup
  ...
  draw-tree
  reset-ticks
  set task-movement-in-progress false
end

to setup-turtles
  let max-width 30  ;; Maximum horizontal spacing

  ; Loop through levels to create turtles in layers
  let layer-ycor 10  ;; Start y-coordinate for the top layer

  let current-layer 0;
  let current-turtle-number 1;

  ; Set up a loop for each level
  repeat levels [
    let nodes-in-this-layer 2 ^ current-layer  ;; Number of nodes in each level (binary tree, doubling each level)
    let horizontal-spacing max-width / nodes-in-this-layer  ;; Calculate horizontal spacing between nodes

    ; Create turtles at calculated positions for each node
    let start-xcor (0 - max-width / 2) + (horizontal-spacing / 2)
    let i 0  ;; Counter for each node in the layer

    ; Create the turtles for the current level
    repeat nodes-in-this-layer [
      let current-xcor start-xcor + (i * horizontal-spacing)
      create-nodes 1 [
        setxy current-xcor layer-ycor
        set layer current-layer  ;; Assign the turtle's layer based on its ycor
        set number current-turtle-number
        set success-probability random-float 1
        set task-assigned false
        set task-completed false
        set color blue
        set shape "circle"
        print (word "Created turtle number: " number " at (" xcor ", " ycor ") in layer " layer)

      ]
      set i i + 1  ;; Increment the counter for the next turtle
      set current-turtle-number current-turtle-number + 1
    ]

    set layer-ycor layer-ycor - 2  ;; Move down to the next layer
    set current-layer current-layer + 1
  ]
end

to draw-tree
  ask turtles with [ layer < max [ layer ] of turtles ] [
    ; Calculate the two child turtle numbers based on the current turtle's number
    let child1 turtles with [ number = ([number] of myself * 2) ]
    let child2 turtles with [ number = ([number] of myself * 2) + 1 ]
    print (word "Turtle " number " is connecting to its children." child1 " and " child2)

    ; Check if both children exist and create links
    if any? child1 [
      create-link-with one-of child1
      print (word "Turtle " number " linked to child turtle " [number] of child1)
    ]
    if any? child2 [
      create-link-with one-of child2
      print (word "Turtle " number " linked to child turtle " [number] of child2)
    ]
  ]
end

to create-task
    create-task-turtles 1 [
      setxy 0 0  ;; Start the task turtle at the bottom of the screen
      set shape "triangle"
      set color yellow  ;; Task turtle is yellow
      set size 1
      set heading 0  ;; Initial heading
      let target-node one-of nodes  ;; Pick a random node
      set task-target target-node
      move-to-node target-node
    ]
end


to move-to-node [target-node]
  ask task-turtles [
    face target-node  ;; Make the task turtle face the target node
    while [distance target-node > 0.5] [
      fd 0.1  ;; Move forward step-by-step
    ]
    ; When the task turtle reaches the node, assign the task
    ask target-node [
      set task-assigned true
      set color yellow  ;; The node turns green when a task is assigned
      print (word "Task assigned to node " number)
    ]
    die  ;; Remove the task turtle once it has reached its target
  ]
end

to attempt-task
  ask nodes with [ task-assigned = true ] [
    let success random-float 1
    ifelse success < success-probability [
      ; Turtle succeeded in solving the task
      print (word "Turtle " number " succeeded!")
      set color green  ;; Change the turtle color to green if successful
      set task-assigned false
    ] [
      ; Turtle failed to solve the task
      print (word "Turtle " number " failed.")
      set color red  ;; Change the turtle color to red if failed
    ]
  ]
end

to go
  repeat tasks_per_tick [
    create-task
  ]
  attempt-task
  tick
